...
import { Leaf, Droplets, Beaker, AlertTriangle, CheckCircle, XCircle, Star, Tractor } from 'lucide-react';
...
const CropSelect = ({ 
  ...
  const getCropSuitability = (crop) => {
    const tempMatch = crop.optimalTemp[0] <= country.temperature && country.temperature <= crop.optimalTemp[1];
    const waterMatch = country.precipitation >= crop.waterNeed * 0.7;
    
    if (tempMatch && waterMatch) return { level: 'excellent', color: 'green', icon: Star };
    if (tempMatch || waterMatch) return { level: 'good', color: 'yellow', icon: CheckCircle };
          ...
          <div className="card mb-4 sm:mb-6 animate-slide-up">
            ...
            <div className="grid gap-3 sm:gap-4 sm:grid-cols-2">
              {countryAlerts.map((alert, index) => (
                <div 
                  key={index}
                  className={`p-3 sm:p-4 rounded-lg border-l-4 ${getAlertBorderColor(alert.severity)} ${getAlertBgColor(alert.severity)}`}
                >
                  ...
                  <div className="space-y-2">
                    <div className="metric-card bg-gray-50 border-l-red-400">
                      ...
                      <span className={`font-medium text-xs sm:text-sm ${
                        crop.optimalTemp[0] <= country.temperature && country.temperature <= crop.optimalTemp[1]
                          ? 'text-green-600' 
                          : 'text-red-600'
                      }`}>
                    ...
                    <div className={`text-center p-2 rounded font-medium text-xs sm:text-sm flex items-center justify-center ${
                      suitability.level === 'excellent' ? 'bg-green-100 text-green-800' :
                      suitability.level === 'good' ? 'bg-yellow-100 text-yellow-800' :
                      'bg-red-100 text-red-800'
                    }`}>
                      <SuitabilityIcon className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
                      {suitability.level === 'excellent' ? 'Excellent Match' :
                       suitability.level === 'good' ? 'Good Match' :
                       'Challenging'}
                    </div>
                  </div>
                </div>
          ...
          {selectedCrop && (
            <div className="border-t-2 pt-6 animate-slide-up">
              ...
              <p className="text-gray-600 text-center mb-4 sm:mb-6 text-sm sm:text-base">
                Choose your farming strategy - each decision affects your results!
              </p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
                ...
                <div 
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={`${irrigation ? 'Disable' : 'Enable'} irrigation system`}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      e.preventDefault();
                      onToggleIrrigation();
                ...
                >
                  <div className="flex items-center justify-center mb-3">
                    ...
                    <span className="font-bold text-lg sm:text-xl">Irrigation System</span>
                  </div>
                ...
                <div 
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={`${fertilizer ? 'Disable' : 'Enable'} chemical fertilizer`}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      e.preventDefault();
                      onToggleFertilizer();
                ...
                >
                  <div className="flex items-center justify-center mb-3">
                    ...
                    <span className="font-bold text-lg sm:text-xl">Chemical Fertilizer</span>
                  </div>
                  <p className="text-center text-gray-600 mb-2 font-medium">
